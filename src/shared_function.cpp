...
// [[Rcpp::export]]
arma::mat cal_breed_pro(arma::mat Pedigree){   //前三列为个体号，父亲，母亲，第四列为品种(1,2,3......未知品种为0)
	int n_ind=Pedigree.n_rows;
	int Sire_id,Dam_id;
	...
	uvec indices = find_unique(arma::nonzeros(Breed));
	int n_breed=indices.size();
    arma::mat Breed_matrix(n_ind,n_breed+1);	
	Breed_matrix.fill(0);
	...
	for(int i=0; i < n_ind; i++){
		...
		if(Sire_id==0&&Dam_id==0){  //父母均未知,
			...
			Breed_matrix(i,Breed[i])=1; //父母未知的情况下,此时品种必须是已知的
				
		}else{                     //父母均已知	

			
			for(int j=0;j<n_breed;j++){
			
			Breed_matrix(i,j+1)=(Breed_matrix(Sire_id-1,j+1)+Breed_matrix(Dam_id-1,j+1))/2; //父母均已知,计算品种j的比例					
		   }
	    }
	}}
