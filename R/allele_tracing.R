...
IND_Father=rep(Father_cross_ped[,1],each=2)
IND_Mother=rep(Mother_cross_ped[,1],each=2)
IND_Unknow=rep(Unknow_cross_ped[,1],each=2)

if(nrow(Mother_cross_ped)==0){IND_Father=NULL;Mother_cross_ped=as.matrix("0")}
if(nrow(Father_cross_ped)==0){IND_Mother=NULL;Father_cross_ped=as.matrix("0")}
if(nrow(Unknow_cross_ped)==0){IND_Unknow=NULL;Unknow_cross_ped=as.matrix("0")}
...
colnames(offspring_status)=c(IND_Father,IND_Mother,IND_Unknow)
